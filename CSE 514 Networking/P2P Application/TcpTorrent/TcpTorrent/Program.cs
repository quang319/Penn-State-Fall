...
namespace TcpTorrent
{
    class Program
    {
            ...
            while (true)
            {
                ...
                switch (userCommand.Item1)
                {
                    ...
                    case "DOWNLOAD":
                        ...
                        Thread myNewThread = new Thread(() => GetDownloadFile(clientState));
                        myNewThread.IsBackground = false;
                        myNewThread.Start();
                        Thread.Sleep(500);
                        //commandPrint();
                        break;
        ...
        private static void GetDownloadFile(StateObject clientState)
        {

            var getLocationCmd = new ClientPassableObject(clientState);
            getLocationCmd.GetFilesLocation(clientState.FileNameToDownload);
            var getLocationClient = new TcpTorrent();
            var getLocationTask = getLocationClient.ClientStart(getLocationCmd);
            ...
            for (int i = 0; i < NoOfSegments; i++)
            {
                ...
                var getFileClient = new TcpTorrent();
                var getFileTask = getFileClient.ClientStart(getFileCmd);
